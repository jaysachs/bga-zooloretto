...
class Elements {
  ...
  static enclosureSpace(player_id: number, enclosure_id: number, enclosure_pos: number) : HTMLElement {
    ...
    pes.forEach((pe : PossibleExchange) =>
      pe.dest.positions.forEach(d =>
        this.addSelectableOnclick(
          Elements.enclosureSpace(this.player_id, pe.dest.enclosure_id, d),
          (evt) =>  {
            let anims: AnimationList = [];
...
class MoveTileFlow extends ZooFlow<PossibleMove[]> {
  ...
  override doStart(possibleMoves: PossibleMove[]) {
    ...
    possibleMoves.forEach((m: PossibleMove) => {
      this.addSelectableOnclick(
        Elements.enclosureSpace(this.player_id, m.src.enclosure_id, m.src.pos),
...
class ZoolorettoGame extends BaseGame<ZGamedatas> {
  ...
  private addStockTile(tile_type: string = 'back') {
    $(IDS.PRIMARY_PILE_TILES).appendChild(this.makeTileSpan(tile_type)!);
  }

  private renderStock() : void {
    let s = Math.min(this.gamedatas.primary_pile_size, 5);
    ...
    var n = Math.min(this.gamedatas.endgame_pile_size, this.gamedatas.lastround ? 5 : 4);
    while (n-- > 0) {
  ...
  private renderTrucks(): void {
    for (let truck of this.gamedatas.trucks) {
      truck.contents.forEach(contents => {
        ...
        if (tileSpan) {
          Elements.truckSpace(truck.truck_id, contents.pos).append(tileSpan);
        }
      });
    ...
    for (let penc of this.gamedatas.player_enclosures) {
      ...
      for (let enc of penc.enclosures) {
        for (let es of enc.spaces) {
          ...
          if (span) {
            Elements.enclosureSpace(penc.player_id, enc.enclosure_id, es.pos).append(span);
          }
        }
      }
    }
