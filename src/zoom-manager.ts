...
const advThrottle = (ctx: any, func: any, delay: number, options = { leading: true, trailing: false }) => {
    let timer: any = null,
      lastRan: any = null,
      trailingArgs: any = null;

    return function (...args : any[]) {

      if (timer) { //called within cooldown period
        lastRan = ctx; //update context
        trailingArgs = args; //save for later
        return;
      }

      if (options.leading) {// if leading
        func.call(ctx, ...args) //call the 1st instance
...
export class ZoomManager {
    ...
    protected zoomControls: HTMLDivElement | undefined;
    protected zoomOutButton: HTMLButtonElement | undefined;
    protected zoomInButton: HTMLButtonElement | undefined;
    ...
    constructor(protected settings: ZoomManagerSettings) {
        ...
        if (settings.smooth ?? true) {
            ...
            settings.element.addEventListener('transitionend', advThrottle(this, () => this.zoomOrDimensionChanged(), this.throttleTime, { leading: true, trailing: true, }));
        }
        ...
        window.addEventListener('resize', advThrottle(this, () => {
        ...
        if (window.ResizeObserver) {
            new ResizeObserver(advThrottle(this, () => this.zoomOrDimensionChanged(), this.throttleTime, { leading: true, trailing: true, })).observe(settings.element);
        }
